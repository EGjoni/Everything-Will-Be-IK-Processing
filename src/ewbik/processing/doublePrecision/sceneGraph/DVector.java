...
public class DVector {
	...
	/**
	 * @return true if any of this DVector's values are NaN. False otherwise. 
	 */
	...
	static public DVector fromAngle(double angle, DVector target) {
		if (target == null) {
			target = new DVector(Math.cos(angle),Math.sin(angle),0);
		} else {
	 ...
	 * @param v2 the x, y, and z components of a DVector object
	 */
	static public DVector sub(PVector v1, PVector v2) {
		return sub(v1, v2, null);
	}
	 ...
	 * @param target DVector in which to store the result
	 */
	static public DVector sub(PVector v1, PVector v2, DVector target) {
		if (target == null) {
			target = new DVector(v1.x - v2.x, v1.y - v2.y, v1.z - v2.z);
		} else {
			target.set(v1.x - v2.x, v1.y - v2.y, v1.z - v2.z);
